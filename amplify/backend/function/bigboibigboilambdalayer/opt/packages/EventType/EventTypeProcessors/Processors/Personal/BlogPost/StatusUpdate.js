...
module.exports = {
  populateMetadata: async evt => {
    ...
    const additionalData = await queryGraphQLData(
      getBlogPost,
      { id: get(evt, 'metadata.sourceMessage.eventData.id') },
      'data.data.getPost'
    );
    return assign(evt, {
  ...
  populatePublishInfo: async evt => {
    ...
    const targetLink =
      includes(['PUBLISHED'], contentStatus) &&
      `https://www.datbdo.com/blogs/${slugify(get(evt, 'metadata.additionalData.title'))}`;
    const publishInfo = {
      icon: {
        type: 'Icon',
        value: 'info',
