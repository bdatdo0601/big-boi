...
module.exports = {
  populateMetadata: async evt => {
    ...
    return assign(evt, {
      metadata: { visibility: isDatTheSender ? 'public' : 'private' },
      content: { githubActionType, isDatTheSender, ...contentData },
    });
  },
  populatePublishInfo: async evt => {
    ...
    const title = has(evt, `content.${formattedActionType}`)
      ? get(evt, `content.${formattedActionType}.title`)
      : capitalize(formattedActionType.replace(/\_/g, ' '));
    ...
    const publishInfo = {
      ...
      targetLink: get(
        evt,
        `content.${formattedActionType}.html_url`,
        get(
          evt,
          `metadata.sourceMessage.${SPECIAL_GITHUB_ACTION_TO_URL[githubActionType]}`,
          get(evt, 'content.repository.html_url')
        )
      ),
