...
export const AppDrawer = ({ open, onClose, routeList }) => {
  ...
  return (
    <Drawer open={open} onClose={onClose} direction="left" style={{ width: 300 }}>
      <div className="bg-popover h-full pb-4 px-4 flex flex-col justify-between overflow-y-scroll">
        <div className="flex flex-col gap-4 relative">
          <div className="flex justify-between items-center sticky top-0 bg-popover pt-4">
            <LogoTitle />
            <IconButton onClick={onClose} style={{ outline: 'none' }} size="large">
          ...
          {!isEmpty(groupedContents) &&
            Object.keys(groupedContents).map((groupedContentKey, index) => (
              <div
                ...
                className="border-b-1 pb-4 mt-2 flex-col flex gap-2 border-popover-foreground"
              >
                {groupedContentKey && (
        ...
        <div>
          <div className="pt-4 border-y-1 pl-2 flex flex-col gap-4 py-4 border-popover-foreground">
            <Switch
              isOn={isDark}
              handleToggle={e => {
            ...
            />
            <Switch
              isOn={globalAnimation}
              handleToggle={e => {
                setGlobalAnimation(e.target.checked);
              }}
              label="Animation"
            />
          </div>
