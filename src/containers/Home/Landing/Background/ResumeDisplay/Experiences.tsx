...
import { useWork } from '@/components/Vitae';

const Experiences: React.FC = () => {
  const experiences = useWork();
  return (
    <ProfileCard header={<h3>Experiences</h3>}>
      <Stepper orientation="vertical" className="text-left" nonLinear>
        {experiences.map(item => (
          <Step key={item.company + item.position} active expanded>
            <StepIcon icon={null} />
            <StepLabel icon={<Avatar width={50} height={50} alt={item.company} src={item.icon} />}>
              <div className="text-input flex flex-col gap-1">
                ...
                <time className="text-base italic">
