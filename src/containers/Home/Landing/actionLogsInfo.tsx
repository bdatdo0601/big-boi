...
const RowRenderer = ({ index, style }: { data: EventMessage[]; index: number; style: React.CSSProperties }) => {
  ...
  return (
    <div style={style}>
      ...
      >
        <div
          className="sticky left-0 flex flex-nowrap gap-2 align-center items-center bg-muted h-full pl-4 pr-1"
        ...
        <div className="flex flex-col gap-1 text-left grow px-1 scroll-auto w-fit" style={animationStyle}>
          <span className="text-base flex flex-nowrap justify-start gap-2 items-center">
            {messages
              .filter(item => item)
              .map((msg, i) =>
                isString(msg) ? (
                  ...
                  <span className="flex flex-nowrap text-nowrap" key={`${msg} ${i}`}>
                    {msg}
                  </span>
                )
              )}
          </span>
          <span className="flex flex-nowrap text-sm text-input italic">
