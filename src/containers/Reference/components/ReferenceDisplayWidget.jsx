...
const TreeReferenceDisplayWidget = ({ data, loading }) => {
  ...
  return (
    <div className={`${level > 0 ? 'border-l-1 pl-2' : ''} ${isOver ? 'border-primary' : 'border-input'}`}>
      <div ref={drop}>
        {level !== 0 && <span className="text-md font-bold text-input px-2 py-1 rounded-lg">{get(data, 'name')}</span>}
        {get(data, 'references', []).length !== 0 && (
          <div className="flex flex-wrap gap-2 my-4">
            {get(data, 'references', []).map(item => (
              <ReferenceRenderer key={get(item, 'id')} reference={item} draggable />
            ))}
          </div>
        )}
      </div>
      <div className="flex flex-wrap gap-8 mt-2">
        {Object.values(get(data, 'children', {}))
          ...
          .map(item => (
            <TreeReferenceDisplayWidget key={get(item, 'name')} data={item} loading={loading} bordered />
          ))}
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="flex justify-between">
        <Switch
          isOn={isDisplayTreeReference}
          handleToggle={e => {
            setIsDisplayTreeReference(e.target.checked);
          }}
          label="Tree View"
        />
        {isDragging && (
