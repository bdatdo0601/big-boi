...
const ResumeIcon =
  (IconComponent: MaterialIconComponent): React.FC =>
  () => <IconComponent sx={{ fontSize: 12 }} />;

const ResumeContactItem: React.FC<{ IconComponent: MaterialIconComponent; name?: string }> = ({
  IconComponent,
  name,
}) => {
  ...
  return (
    <div ref={ref} className="w-[8.5in] p-[5mm] mx-auto bg-white text-black shadow-lg">
      <div className="printable font-sans text-base leading-relaxed flex-col flex gap-[1mm]">
        <header className="text-center flex-col flex">
          ...
          <p className="flex items-center justify-center text-center gap-[1mm]">
            ...
            <ResumeContactItem
              ...
              name={resume.basic.profiles.find(item => item.network === 'GitHub')?.username}
            />
            <span className="text-xs text-black">{' | '}</span>
            ...
            <ResumeContactItem
              IconComponent={Place}
              name={`${resume.basic.location.city}, ${resume.basic.location.region}`}
            />
          </p>
        </header>
        <section>
          <h2 className="text-sm font-bold border-b-2 border-black mb-1 text-center text-black">
            {'Competencies'.toLocaleUpperCase()}
          </h2>
              ...
              <div key={workItem.company + workItem.position}>
                ...
                <ul className="list-disc pl-4">
                  {workItem.highlights.map(item => (
                    <li className="text-xs text-black" key={item}>
                      {item}
                    </li>
                  ))}
                </ul>
        ...
        <section>
          <h2 className="text-sm font-bold border-b-2 border-black mb-1 text-center text-black">
            {'Research & Side Projects'.toLocaleUpperCase()}
          </h2>
              ...
              <div key={educationItem.institution}>
                ...
                <span className="flex flex-row justify-between">
                  <span className="text-xs italic">
                    Minors: {(educationItem.minors || ['Applied Mathematics', 'Computer Networking']).join(' & ')}
                  </span>
        ...
        <section>
          <h2 className="text-sm font-bold border-b-2 border-black mb-1 text-center text-black">
            {'Awards & Affilliations'.toLocaleUpperCase()}
          </h2>
          <div className="flex flex-col gap-1">
