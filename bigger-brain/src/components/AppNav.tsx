...
export default function AppNav({
  ...
  tree: FileTree[];
  flattenTree: FlattenFileTreeWithData[];
    ...
    <div id="AppNav" className="w-full flex flex-col">
      <div className="top-0 w-full bg-accent">
        <div className="flex flex-row flex-wrap justify-between p-4 items-center gap-2">
          ...
          <div className="flex flex-row gap-2 items-center">
            <Search
              items={flattenTree.map(item => ({ name: item.name, content: item.content, path: item.path }))}
              onResultSelect={result => {
                router.push(`/docs/doc/${result.path}`);
              }}
            />
            <button onClick={toggleGraphDrawer} className="bg-primary rounded-full p-1.5 hover:cursor-pointer">
              <MapIcon />
            </button>
          </div>
        </div>
      ...
      <div className="w-full px-1 relative">
        <div className="min-lg:fixed min-lg:right-2 min-lg:top-24 max-lg:mx-auto max-lg:mt-2 overflow-y-auto border-2 rounded-md p-4 bg-popover min-w-[250px]">
          <div className="flex flex-row justify-between items-center flex-wrap gap-10">
            ...
            <button
              onClick={() => {
                setIsTreeOpen(prevState => !prevState);
              }}
              className="bg-primary rounded-full p-1.5 hover:cursor-pointer float-right"
            >
              {isTreeOpen ? <ArrowDropDown /> : <ArrowLeft />}
            </button>
          </div>
      ...
      >
        <div className="w-full h-full bg-popover flex flex-col items-center min-sm:p-4 p-2">
          <button
            className="fixed top-2 left-2 p-2 bg-secondary rounded-full z-50 hover:cursor-pointer"
          ...
          <div className="overflow-hidden h-full grow w-full border-primary border-1 rounded-md">
            <GraphRenderer
              onNodeClick={toggleGraphDrawer}
              items={flattenTree.map(item => ({ title: item.name, backlinks: item.backlinks, path: item.path }))}
            />
          </div>
        </div>
      </Drawer>
