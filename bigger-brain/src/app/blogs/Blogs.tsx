...
const Blogs = ({ posts }: { posts: BlogPostWithSlug[] }) => {
  return (
    <div className="max-w-[1200px] mx-auto px-4 py-8 flex flex-col gap-2">
      ...
      <p className="mb-8 mt-4">Aggregation of my thoughts from various sources</p>
      <div className="w-full">
        ...
        >
          <Masonry>
            {orderBy(posts, "updatedAt" , "desc").map((post, index) => <BlogPostCard key={post.title} data={post} index={index} width={350} />)}
          </Masonry>
        </TypedResponsiveMasonry>
      </div>
    </div>
