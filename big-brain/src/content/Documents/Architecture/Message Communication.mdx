...
- The other ingress source is Rest API via API Gateway. This only required an API Key to authenticate to open for wide variety of integration
  - For Webhook actions, it will be proxied through [IFTTT](https://maker.ifttt.com/) (refer to [[Webhook Proxy]])
...
- Every message will first need to be determined its `eventType` then go through proper transformation to become a `message` that system can understand
- These categorizer are currently just a giant JSON object with transform, validate functions

#### Open Questions

- There will be future messages with different event type coming in. How do we keep this maintainable and flexible? (a solution here might be `node-transformer-json`)
